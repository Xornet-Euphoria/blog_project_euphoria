...
RSA暗号の公開鍵$N$が2つの素数の積になっているので素因数分解は簡単に出来る。この時、秘密鍵$d$はであり、である事に注意して計算すると無事に復号出来る。
...
この方法はである事からとなるので、と定義するとになる。したがって、$y_i = a^iy_0$になる。よってである事から、$i+j = i' + j'$となる添字の組で$i \neq i' \land j \neq j'$となるものを用意し、を計算すると$M$を法として0になる、つまり$M$の倍数になるはずなので、そのようなものを複数用意して最大公約数を取り、それを素数になるまで割っていけば$M$が導出されるはずである。

前述の動画リンクではこれを6つの乱数で行っていたが、5つでも大して変わらない。$i+j=4$をで用意し、$i+j=5$をで用意した。
...
この問題の乱数生成はである。これに注目すると、もしが$m$より小さければ偶数(最下位bitが0)でそうでなければ$m \leq 2x_i \lt 2m$より、であり、$m$が奇数より、も奇数(最下位bitが1)になる。
