...
RSA暗号の公開鍵$N$が2つの素数の積になっているので素因数分解は簡単に出来る。この時、秘密鍵$d$は$d := e^{-1} \bmod \phi(N)$であり、$\phi(p^2) = p(p-1)$である事に注意して計算すると無事に復号出来る。
...
この方法は$x_{i+1} = a*x_{i} + b \bmod m$である事から$x_{i+1} - x_i = a(x_{i} - x_{i-1})$となるので、$y_i := x_{i+1} - x_i$と定義すると$y_{i+1} = ay_i$になる。したがって、$y_i = a^iy_0$になる。よって$y_iy_j = a^{i+j}y_0^2$である事から、$i+j = i' + j'$となる添字の組で$i \neq i' \land j \neq j'$となるものを用意し、$y_iy_j - y_{i'}y_{j'}$を計算すると$M$を法として0になる、つまり$M$の倍数になるはずなので、そのようなものを複数用意して最大公約数を取り、それを素数になるまで割っていけば$M$が導出されるはずである。

前述の動画リンクではこれを6つの乱数で行っていたが、5つでも大して変わらない。$i+j=4$を$(i, j) = (1, 3), (2, 2)$で用意し、$i+j=5$を$(1, 4), (2, 3)$で用意した。
...
この問題の乱数生成は$x_{i+1} = 2x_{i} \bmod m$である。これに注目すると、もし$2x_{i}$が$m$より小さければ偶数(最下位bitが0)でそうでなければ$m \leq 2x_i \lt 2m$より、$x_{i+1} = 2x_i - m$であり、$m$が奇数より、$x_{i+1}$も奇数(最下位bitが1)になる。

ここで、ある推定値$x+\alpha$を用意して乱数生成手順を経ていくと、次の値は$M$との大小関係だけで決定されるので、実際の値に近ければ似たような乱数列になり、先程の性質からLSBも似たような並びになると思われる。
