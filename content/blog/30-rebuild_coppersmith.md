...
この基底行列に左から係数ベクトル$(a_0, a_1X,\dots, a_{d-1}) \in \mathbb Z^d$を掛けると$(b_0, b_1X, \dots, b_{d-1}X^{d-1})$が現れる。これらの関係は次のようになっている。

$$
b_iX = \sum_{j=0}^{d-1} a_jf^{(i)}_{k,l}(xX) \ \ \ (\deg f\_{k,l} = l+ke = j)
$$
