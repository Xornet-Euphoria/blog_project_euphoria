...
最初に与えるシェアを{{katex(body="s_1'")}}、その提出によって復元された秘密を{{katex(body="s'")}}とおくと{{katex(body="s' \equiv \frac{s_1'^3}{s_2s_3} \bmod p")}}が成り立つ。ここから{{katex(body="s_2s_3")}}を求める事が出来る。
...
さて、問題は普通だったら`c = flag * G`みたいな形で`flag`が特定出来る事が多いのだが、今回はそうもいかない。具体的にはフラグを複数(6つ)に分割して{{katex(body="m_i")}}のようにおき、同様に`G`の各要素も{{katex(body="G_i")}}のようにおくと{{katex(body="c = \sum_{i=1}^6 m_iG_i")}}のようになっている。
...
この格子の体積はだいたい{{katex(body="2^{512}")}}で、{{katex(body="m_i")}}は64bitなのでLLLで出てくる小さな基底の上界{{katex(body="{2^{512}}^{1/7} \approx 2^{73}")}}よりも小さく、遠くもない。よって上手く出てくれそうである。
