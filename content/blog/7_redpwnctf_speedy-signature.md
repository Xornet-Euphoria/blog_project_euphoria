...
1. `r`は楕円曲線上の点のx座標なので仮に残りの数字が`r`であれば、$y^2 \equiv r^3 + ar + b \bmod p$となる$y$が存在するはずである。
2. これは平方剰余を使って判定出来る。もし、右辺が$p$の平方剰余でない場合はこのような$y$は存在しないため`r`では無い、つまり`s`である事がわかる
...
ここで$\delta$は$-4095 \leq \delta \leq 4095$を満たすことから、$P_1$が与えられたら$\delta$をこの範囲で探索することで$P_2$を特定することが出来る。$\delta$はそこまで広くない上に計算した$P_2$のx座標が$r_2$になるため、既に与えられている`s2`と署名の検証をすることで、現実的な範囲とアルゴリズムで$r_2$を特定することができる。

では肝心の$P_1$の特定だが、これは方程式$y^2 \equiv r_1^3 + ar_1 + b \bmod p$を満たすような$y$を求めれば良い。単純にTonelli-Shanksのアルゴリズムでも良いが、NIST P-256曲線で使われている素数は$p = 4k+3$の形なのでもっと簡単に求めることが出来る(フェルマーの小定理を適用するだけなので略)。

これで求まる$y$は2通りなのでこれら2つに対してそれぞれこの探索を行うことで$r_2$を求めることが出来る。

※下記コードはyの対称性を使って上手く探索の計算量を抑えようとしたんですが、どうも半分ぐらいの確率で失敗します。直すのと検証面倒なのでそのままです。

### 1回目の秘密鍵の特定

前述の手順では$P_1$に$G$を足していくことで$P_2$のx座標として正しいものを特定したため、この足した回数、つまり$\delta$も特定出来たことになる。ここで2つの署名`s_1, s_2`に関して次のような関係がある。

$$
s_1 \equiv \frac{z + r_1d}{k_1} \bmod n, \ s_2 \equiv \frac{z + r_2d}{k_1 + \delta} \bmod n
$$

但し$n$は曲線の位数、$z$は署名したメッセージをSHA-256にかけたもので、$d$は秘密鍵である。

2つの未知数$k_1, d$に対して2つの式があるのでどちらも特定可能である。具体的には次のようにすることで$d$を特定出来る。
...
前節で$d$が求まったので1回目の$k_1$も求めることが出来る。2回目以降の署名も`k`に4096までの自然数を足したものが`k_1, k_2`として使われることは変わらないため、1回目で求めた`k_1`を$k_0$とおくと、以降の`k_1, k_2`対して次が成り立つ。

$$
k_0 - 4095 \leq k_1, k_2 \leq k_0 + 4095
$$

2回目以降も正しい署名を1つは手に入れることが出来、その署名で使われた`k_1`はこの範囲に収まる事から事前にこの範囲で$k_1G$を計算しておき、x座標をキーとし、$k_0$との差を値とした辞書を作っておけば2回目以降特定出来た`r_1`はこの辞書のキーに含まれ、`k_1`を特定することが出来る。

`k_1`が求まったということは$d$も求めることが出来るため、2回目以降の署名はこれを提出すれば良い。
