...
We participated in N1CTF as ./Vespiary and were at 7th place last week. I solved 'checkin (crypto)' challenge so write its solution.
...
It looks like a normal RSA challenge with some hints(`h` and `p0`) and their expressions are here.
...
In these expressions, {{katex(body="\gg")}} means right bit shift.

## Solution

I know 22 leading bits of {{katex(body="p")}}, so some leading bits of {{katex(body="q")}} are known too. In my calculation and experiments, 21 leading bits of {{katex(body="q")}} are recoverd. Using these known bits, I rewrited {{katex(body="p,q")}} as follows.
...
From the definition of {{katex(body="h")}}, {{katex(body="x")}} is one of the root of {{katex(body="f(x) = x^2 - xh + 1 \mod n")}}. Substituting {{katex(body="x")}} to {{katex(body="x_0 + x_1")}}, a polynomial that has {{katex(body="x_1")}} as a root appears and its degree is 2. So we can use Coppersmith's Attack!!
...
`xcrypto` is my crypto library. `xcrypto.rsa.p_plus_q_to_pq` can calculate {{katex(body="p,q")}} from {{katex(body="N=pq, p+q")}}. In this challenge, I used it to calculate {{katex(body="2021p, 1120q")}} from {{katex(body="2021\times 1120 N, x")}}.
