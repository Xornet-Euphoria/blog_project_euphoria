...
ここで上の式の$x$については$x(x^2 + a)$と分解する事が出来、2次の項は下の式を代入する事で消去出来る。すると1次の項だけ残るので両辺2乗して強引に2次式にしてから再度代入すれば$x$の項を消去することが出来るので後はこれをSagemathの`roots()`メソッドに解かせて$y$を求め、下の式に代入したものの平方根を求めて終わり。
