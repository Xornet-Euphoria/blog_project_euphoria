...
(走者でも無いのに)完走した感想ですが、「自作ライブラリ最高!!」という気持ちになりました。どの問題もRSAでかつ素因数分解する問題だったので`p,q`を与えたら復号までやってくれる`dec_pq()`という関数を用意しておいたのが生きました。

また、いつもいきあたりばったりで解いているのでSagemathとPythonのコードが混在しており、あまり良いソルバを書けたとは言えませんが、Speedrunでは意外と効果がありました。

vimを使いこなせるわけでもなく、VSCodeのショートカットキーも全然使えないので、速解きには向いていないと思ってましたが、思ったより速く解けた問題があって嬉しかったです(でも普通のCTFでこれが要求されたらキレると思う、こういう企画でやるのが最適)。

SECCON CTF 2021で作問したこともあり、少しだけ運営陣の様子を眺めていましたが、どうやら準備が大変だったようです。運営のみなさんお疲れ様でした。そしてありがとうございました。
